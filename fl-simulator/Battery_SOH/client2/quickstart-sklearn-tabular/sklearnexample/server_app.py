...
def weighted_average(metrics: List[Tuple[int, Metrics]]) -> Dict[str, Scalar]:
    ...
    # Initialize the metric keys from the first client's metrics
    for _, all_metrics_dict in metrics:
        for single_metric, value in all_metrics_dict.items():
            if isinstance(value, (float, int)):
                metrics_lists[single_metric] = []
        break

    # Calculate weighted metrics list
    ...
    # This prints the aggregated result for each round's evaluation
    print(f"Aggregated Metrics (Round End): {weighted_metrics}")
